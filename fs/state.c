...
static pthread_rwlock_t rwlock_table[INODE_TABLE_SIZE];
...
void state_init() {
    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        freeinode_ts[i] = FREE;
        pthread_rwlock_init(&rwlock_table[i], NULL);
    }
...
void state_destroy() { /* Destroy all locks */
    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        pthread_rwlock_destroy(&rwlock_table[i]);
    }
}
    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                for (int i = 0; i < DIRECT_DATA_BLOCKS; i++) inode_table[inumber].i_data_block_direct[i] = -1; // Set entire table to -1
                inode_table[inumber].i_data_block_indirect = -1; // Set indirect block to -1
                
                inode_table[inumber].i_data_block_direct[0] = b; // Except first block which has been allocated
...
int inode_delete(int inumber) {
    ...
    if (inode_table[inumber].i_size > 0) {
        for (int i = 0; i < DIRECT_DATA_BLOCKS; i++) { // Freeing the direct data blocks
            if (inode_table[inumber].i_data_block_direct[i] != -1 && data_block_free(inode_table[inumber].i_data_block_direct[i]) == -1) return -1;
        }

        if (inode_table[inumber].i_data_block_indirect != -1) { // Freeing indirect data blocks
            int *data_block_content = data_block_get(inode_table[inumber].i_data_block_indirect);

            for (int i = 0; i < BLOCK_SIZE / sizeof(int); i++) {
                if (data_block_content[i] != -1 && data_block_free(data_block_content[i]) == -1) return -1;
            }

            if (data_block_free(inode_table[inumber].i_data_block_indirect) == -1) return -1;
        }
    }
...
int inode_lock(int inumber) {
    if (pthread_rwlock_wrlock(&rwlock_table[inumber]) == -1) return -1;

    return 0;
}

int inode_unlock(int inumber) {
    if (pthread_rwlock_unlock(&rwlock_table[inumber]) == -1) return -1;
...
int add_dir_entry(int inumber, int sub_inumber, char const *sub_name) {
    ...
    dir_entry_t *dir_entry =
        (dir_entry_t *)data_block_get(inode_table[inumber].i_data_block_direct[0]);
    if (dir_entry == NULL) {
        return -1;
    }
...
/* Returns index of an inode's nth block
 * Inputs:
 * 	 - inode
 *   - n
 * Returns: index to block if sucessful, -1 otherwise
 */
int inode_get_block_index(inode_t *inode, int block_order) {
    int index = -1;

    if (block_order > 0 && block_order <= DIRECT_DATA_BLOCKS) {
        return inode->i_data_block_direct[block_order - 1];
    } else if (block_order > DIRECT_DATA_BLOCKS && inode->i_data_block_indirect != -1) {
        int* indirect_block_content = (int*) data_block_get(inode->i_data_block_indirect);
        
        return indirect_block_content[block_order - DIRECT_DATA_BLOCKS - 1];
    }
